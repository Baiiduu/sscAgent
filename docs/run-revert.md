...
当前 agent 会按每一�?`runSession()` 记录文件变更，并支持一次回退一个已完成�?run�?
## 存储位置
...
每个 snapshot 目录都是某个 workspace 对应的内�?bare git 存储。它不是项目本身�?git 仓库。不要直接读�?`objects/` 文件，应通过 snapshot service 的接口访问�?
## 工作原理

每次 `runSession()` 开始时，harness 会创建一�?`SessionRun`，并记录本次 run 开始前�?baseline 快照�?
```ts
const baselineHash = await runtime.snapshot.track({ workspace: session.workspace })
```

本次 run 中产生的消息和文件快照都会绑定同一�?`runID`�?
执行 revert 时，系统会：

1. 如果没有指定 `runID`，选择最近一次已完成�?run�?2. 记录当前 workspace 状态，作为后续 `unrevert` 的恢复快照�?3. 找到所�?run 产生的全�?snapshots�?4. 将被修改的文件恢复到�?run 修改前的状态，并删除该 run 新增的文件�?5. �?run 标记�?`reverted`，并�?session 上记�?revert 元数据�?
`unrevert` 会把 workspace 恢复�?revert 前记录的恢复快照，并清空 session �?revert 状态�?
## 回退最近一次已完成 run
